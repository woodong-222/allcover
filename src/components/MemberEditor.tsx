 ...
 *
 * 참여자 목록은 줄바꿈되는 칩(flex-wrap)이다. 이름이 보통 2~3글자여서 한 줄 전체 폭을 쓰는
 * 입력행으로 두면 멤버가 8~10명(볼링 모임 기본 규모)만 돼도 세로 스크롤을 통째로 잡아먹는다.
 * 칩 이름을 탭하면 그 자리에서 바로 수정할 수 있다.
 *
  ...
  return (
    ...
    >
      {editing ? (
        ...
        /* 실제 탭 대상은 칩 컨테이너가 아니라 이 버튼이다. 컨테이너에만 44px 을 주면
           한 글자 이름에서 버튼이 28×36px 이 되어 최소 탭 영역에 못 미친다. */
        <button
          type="button"
