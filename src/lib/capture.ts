 ...
 *
 * 캡처 대상 서브트리(ResultCard 등)는 src/index.css 의 hex CSS 변수(--card-*)만 써야 한다.
 * Tailwind v4 기본 팔레트는 oklch() 색인데, html-to-image 가 노드를 SVG <foreignObject> 로
 * 직렬화할 때 oklch() 가 깨질 수 있다. 캡처 대상 안에 새 클래스를 추가할 때도 마찬가지다.
 *
 * captureNode 는 결과 화면 진입 시 미리 호출해 blob 을 준비해두고, 공유 버튼 클릭 핸들러는
 * 그 blob 을 받아 await 없이 즉시 navigator.share 를 호출해야 한다. 이유는 share.ts 상단 주석 참고.
 *
 * 웹폰트가 캡처에서 누락되지 않도록 getFontEmbedCSS 로 계산한 fontEmbedCSS 를 넘긴다.
 */
...
export async function captureNode(node: HTMLElement): Promise<Blob> {
  ...
  try {
    ...
    // 폰트 임베드는 최선 노력이다. 계산이 실패해도 캡처 자체는 계속 시도한다.
